...
export class DatabaseService {
	...
	/**
	 * 导入对话。
	 * @param overwrite - false（默认）：已存在的对话跳过、仅追加新的（安全，永不覆盖当前数据）；
	 *                    true：已存在的对话整体覆盖（含消息），新的追加。备份恢复时用 true。
	 */
	static async importConversations(
		data: { conv: DatabaseConversation; messages: DatabaseMessage[] }[],
		overwrite = false
	): Promise<{
		imported: DatabaseConversation[];
		skipped: DatabaseConversation[];
		updated: DatabaseConversation[];
	}> {
			...
			async () => {
				for (const item of data) {
					...
					if (existing) {
						...
						// 覆盖：先清掉该对话旧消息，再整体写回备份版本。
						await db[IDXDB_TABLES.messages].where('convId').equals(conv.id).delete();
