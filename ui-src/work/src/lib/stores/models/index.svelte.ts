...
class ModelsStore implements ModelPropsHost, ModelStatusHost {
	...
	get singleModelName(): string | null {
		...
		// 零模型哨兵（外壳懒加载模式）：端口上一个模型都没有，但界面仍要报出
		// 「上次用的那个」名字（状态由调用方标成"未加载"）。否则聊天框会退回
		// "Select model"，用户会以为自己的配置丢了。
		if (serverStore.isSentinel) return lastModelStore.current?.name ?? null;

		// 哨兵的 alias 是 llama-server、path 是 none，别当成真模型名漏出去
		if (props?.model_alias && props.model_alias !== 'llama-server') return props.model_alias;

		if (!props?.model_path || props.model_path === 'none') return null;
