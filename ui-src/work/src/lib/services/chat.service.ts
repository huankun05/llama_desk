...
export class ChatService {
	...
	static async sendMessage(
		...
		try {
			...
			// 端口上可能压根没有模型：① 应用刚打开时外壳只起了一个「零模型哨兵」，
			// 模型要等第一次发言才加载；② 空闲看门狗把没人用的模型卸掉了（默认 5 分钟）。
			// 两种情况直连上去都只会得到「服务器未运行」这种误导性报错 —— 先按需把模型
			// 拉起来再发。最多等 2 分钟（模型加载时间），期间界面已经处于「生成中」，有反馈。
			// 该调用绝不抛异常：manager 不在、端口上本来就有模型、老版本没有 /api/switch，
			// 都会安静放行，不新增失败模式。
			await ManagerService.ensureModelReady({ signal });
