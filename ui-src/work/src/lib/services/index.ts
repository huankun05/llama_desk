...
/**
 * **ManagerService** - 本地管理器 manager.py（127.0.0.1:8090）通信层
 *
 * 包装「列出磁盘 GGUF / 列出实例 / 一键换模型（腾出端口后启动）/ 停止实例」
 * 以及实时系统指标。官方 WebUI 的模型加载/卸载接口只在 llama.cpp 的 ROUTER
 * 模式下有效，本项目跑的是单模型模式，所以聊天框的模型选择器走这里。
 */
export { MANAGER_BASE, ManagerError, ManagerService } from './manager.service';
export type {
	ManagerFitPlan,
	ManagerInstance,
