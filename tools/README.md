...
| `probe_download_page.mjs` | **模型下载页（第 2 批 A）端到端验证**（产出 `diag/shots-20260923-download/`）：`#/download` 直达 → 侧边栏 `svg.lucide-download` 入口点击跳转 → 真实 HF 搜索 → **skip 偏移 API 断言（skip=0 与 skip=5 首条不同）** → **「加载更多」翻页（31→61 行，⚠️ 第二页走外网 2~5s+，必须 waitForFunction 轮询行数、固定 sleep 会假阴性）** → 展开仓库拿量化清单（大小 + 可上卡徽章）→ **大小筛选 chips（'> 6 GB' 出空态）+ 排序方向切换（GB 序列断言降/升序）** → 点最小量化下载 → **下载任务卡片 + 非零进度** → 取消 →「已取消」→ **「删除记录」点击后卡片消失** → **磁盘余量检查直测**（POST total=99TB → 400 + 「磁盘空间不足」）→ **真实下载到完成（顺带回归断点续传：场景 7 取消的 sidecar 被场景 9 续完）→ 「加载模型」点击跳转对话页**。26 项断言。⚠️ 从仓库根跑；⚠️ 重跑前删 `models/from-hf/<repo>/` 并重启 manager（文件已完整会让场景 7 的取消秒完成而假失败）；⚠️ manager 用 `HF_API_BASE=https://hf-mirror.com` 切镜像。 |
