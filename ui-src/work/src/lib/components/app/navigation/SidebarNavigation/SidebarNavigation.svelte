<script lang="ts">
	...
	/** 会话列表顶部的内联搜索框（取代原先独占一行/独占一页的搜索） */
	let searchInputRef = $state<HTMLInputElement | null>(null);
	let searchQuery = $state('');
	/** 没有会话就不展示搜索框 —— 也没东西可搜 */
	const hasConversations = $derived(conversationsStore.conversations.length > 0);
	/** 只要输入了关键词就切到「搜索结果」视图（复用官方组件） */
	const isSearchModeActive = $derived(searchQuery.trim().length > 0);

	/** 侧栏的 Search 图标与 cmd+K 都只把焦点送进顶部搜索框 */
	function focusSearch() {
		uiStore.isSidebarExpanded = true;

		if (!hasConversations) return;

		setTimeout(() => searchInputRef?.focus({ preventScroll: true }), 0);
	}
	...
	let filteredConversations = $derived.by(() => {
		if (!isSearchModeActive) return conversationsStore.conversations;
