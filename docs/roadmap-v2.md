...
>
> **下载页待完善项**（用户 2026-09-23 反馈「不能按模型大小筛选，操作不够多」）：
> - 结果列表**按大小筛选/过滤**（如 <3GB / 3~6GB / >6GB，或自由区间），配合徽章口径直接可跑；
> - **按量化档筛选**（Q4_K_M / Q8_0 / …）与**按大小排序**（目前只有 repo 级三排序，展开后的文件列表无排序）；
> - 结果**分页/加载更多**（现在一次 limit 条）；`/api/hf-search` 补 `min_size/max_size` 参数更干净；
> - 任务操作补充：**打开所在文件夹**、**删除已下载任务记录**、下载前磁盘余量检查（roadmap §A 表里已列、尚未实现）；
