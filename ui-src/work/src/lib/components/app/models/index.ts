...
/**
 * **ModelLoaderDropdown** - 聊天框的「模型装载」下拉（非路由模式）
 *
 * 非路由模式下官方 ModelsSelectorDropdown 点开的是「模型详情」，无法换模型。
 * 这个组件改为列出 manager.py 托管的全部可用 GGUF，点一个就执行
 * 「停掉旧实例 → 腾出端口 → 用新模型启动」，并轮询 /health 等新模型就绪后自动刷新。
 */
export { default as ModelLoaderDropdown } from './ModelLoaderDropdown.svelte';
