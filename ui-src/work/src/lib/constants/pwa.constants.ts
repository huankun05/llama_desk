...
export const SVELTEKIT_PWA_OPTIONS: SvelteKitPWAOptions = {
	...
	workbox: {
		// ⚠️ NEVER precache the HTML app shell.
		//
		// @vite-pwa/sveltekit force-adds a `prerendered/**/*.{html,json}` glob
		// (see buildGlobPatterns in its dist/index.mjs), so the prerendered
		// `pages/index.html` lands in the precache manifest as `./` — and
		// Workbox then serves that frozen copy for every navigation to `/`.
		// `navigateFallback: ''` below does NOT prevent this: it only disables
		// the NavigationRoute, not the precache route.
		//
		// Because our shell pins a concrete `overlay.js?v=N`, a frozen shell
		// freezes the whole UI: the runtime overlay keeps pointing at a stale
		// `?v=` URL, so every later overlay deploy is silently ignored.
		// This really happened (2026-09-21): the SW held a `?v=39` shell while
		// the server was already serving `?v=42`, which surfaced as
		// "改了不生效" + leftover English strings on the performance page.
		//
		// Keeping navigations on the network costs nothing on localhost and is
		// exactly what we want: the shell is 14 KB and always fresh.
		globIgnores: ['**/*.html'],
