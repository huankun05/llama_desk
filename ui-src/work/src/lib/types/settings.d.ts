...
export interface SettingsEntry {
	...
	/**
	 * Options rendered for SELECT fields.
	 *
	 * `icon` 是**可选**的：渲染处（SettingsChatFields.svelte）本来就是
	 * `{#if option.icon}` 判空再画，语言选择这类选项天然没有图标。
	 * 之前写成必填，导致 settings.constants.ts 里 2 个既有类型错误一直挂着，
	 * 把真正的错误淹在 svelte-check 输出里。
	 */
