...
// 只引入 common 语言集（37 种常用语言），不要用裸 'highlight.js'。
// 裸导入会把全部 190+ 种语言的 grammar 打进 bundle —— 实测占了整个前端包约 36%
// （8.5MB 里约 3MB），而聊天里出现的代码 99% 就在 common 覆盖的范围内。
// 语言不在 common 时 highlightCode() 会退化为 highlightAuto 或纯文本，不会报错。
