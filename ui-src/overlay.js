(function(){
  ...
  var DICT = {
    ...
    'App info': '应用信息',
    'App updates': '应用壳更新',
    'llama.cpp updates': 'llama.cpp 更新',
    'Check for app updates': '检查应用壳更新',
    'This app (the shell) and the llama.cpp engine below are updated through two separate channels. Shell updates are published on the project\'s GitHub releases page: download the new llama-desk.exe and replace the old file — your config, models, chat history and backups all live outside the exe and are kept as-is. Restart the app after replacing.': '本应用（外壳）与下方的 llama.cpp 引擎走两条独立的更新通道。外壳更新发布在项目 GitHub 的 releases 页面：下载新版 llama-desk.exe 替换旧文件即可 —— 你的配置、模型、聊天记录与备份都在 exe 之外，替换后原样保留。替换完重启应用即生效',
    'Service & logs': '服务与日志',
    ...
    'When enabled, the app checks GitHub at startup and notifies you when a new build is available. Downloading and installing always requires your confirmation.': '开启后应用启动时会检查 GitHub，发现新版本会在应用内弹出提示。下载与安装始终需要你确认后才会进行',
    'llama.cpp update available:': '发现 llama.cpp 新版本：',
    'View update': '查看更新',
    'Check for updates': '检查更新',
    'Download & install update': '下载并安装更新',
    'Checking…': '检查中…',
    'Updating llama.cpp…': '正在更新 llama.cpp…',
    'You can keep using the app; a system notification will tell you when it is done.': '期间可以继续使用应用，更新结束会有系统通知',
    'The running llama-desk.exe was built before this interface existed. Rebuild the shell (': '当前运行的 llama-desk.exe 是本功能之前编译的旧版。重建外壳（',
    ') to see version numbers, update controls and the simplified tray.': '）后即可看到版本号、更新控件与精简后的托盘',
