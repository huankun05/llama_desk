...
> **状态（2026-09-25）**：第 0~2 批整体关闭，第 3~4 批（GPU 健康面板、一键基准、
> 拆 manager 包、量化工具箱、模型标签）待做。本文已转为**历史记事**——新任务
> 的方案与验收按时间顺序追加在文末，查根因/验收细节先看文末索引，文档地图见
> `docs/INDEX.md`。**面向用户的版本变更记录已迁移至根目录 `CHANGELOG.md`**
> （每轮开发记 Unreleased，发版归档）。
...
上一轮（`docs/archive/diag/architecture-review.md`，已归档）的结论是：**架构是干净的，真正的缺口是"模型从哪来"** —— 整条链上唯一还需要你动手的环节。
